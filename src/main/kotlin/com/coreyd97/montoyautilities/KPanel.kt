...
abstract class Container: JPanel(GridBagLayout()), Element {
    ...
    protected fun <T: Element> initComponent(tag: T, gbc: GridBagConstraints, init: T.() -> Unit = {}): T {
        tag.init()
        val gbc = gbc.clone() as GridBagConstraints
        if(tag is Container && gbc.insets == null){
...
//        }
    ...
    fun toggleButtonVetoableByPreference(prefKey: String,
                                         offText: String, onText: String,
                                         onToggleAttempt: (Boolean) -> Boolean,
                                         weightX: Number = 0.0,
                                         init: KToggleButton.() -> Unit = {}): KToggleButton {
        ...
        val onToggle: (Boolean) -> Boolean = { new ->
            val shouldCommit = onToggleAttempt.invoke(new)
            if(shouldCommit) pref = new
            shouldCommit
        }
        button = toggleButtonVetoable(offText, onText, pref, weightX, onToggle, init)
        return button
    }
    ...
    fun checkBoxByPreference(label: String = "", prefKey: String,
        ...
        checkBox = checkBox(label, false, onToggle, weightX, init)
        checkBox.isSelected = pref
        return checkBox
    }
    ...
    fun textByPreference(prefKey: String, label: String? = null, weightX: Number = 1.0, init: KField.() -> Unit = {}): KField {
    ...
    inline fun <reified T> dropdownByPreference(prefKey: String, options: Array<T>, weightX: Number? = 0.0, noinline init: KDropdown<T>.() -> Unit = {}): KDropdown<T> {
        lateinit var dropdown: KDropdown<T>
        var preference by PreferenceProxy(prefKey, serializer = serializer<T>()) { _, new ->
            val f = dropdown
        ...
        dropdown.addItemListener { e ->
            preference = e.item as T
        }
        return dropdown
    }


    fun row(title: String? = null, weightX: Number? = 1.0, weightY: Number? = null, init: KRow.() -> Unit): KRow {
        return initComponent(KRow(title), weightX, weightY, init)
    }
    
    fun column(title: String? = null, weightX: Number? = null, weightY: Number? = 1.0, init: KCol.() -> Unit): KCol {
        return initComponent(KCol(title), weightX, weightY, init)
    }

    fun grid(rows: Int, cols: Int, title: String? = null, weightX: Number? = 1.0, weightY: Number? = null, init: KGrid.() -> Unit): KGrid {
        val panel = KGrid(title, rows, cols)
        return initComponent(panel, weightX, weightY, init)
    }

    fun panel(title: String? = null, weightX: Number? = 1.0, weightY: Number? = null, init: KPanel.() -> Unit): KPanel {
...
open class KPanel internal constructor(title: String? = null, childAlignment: Alignment? = null) : Container() {
    ...
    open fun add(comp: Component, weightX: Number? = null, weightY: Number? = null): Component {
        val gbc = gbc.clone() as GridBagConstraints
        if(weightX != null) gbc.weightx = weightX.toDouble()
        if(weightY != null) gbc.weighty = weightY.toDouble()
        add(comp, gbc)
...
class KRow internal constructor(title: String? = null) : KPanel(title) {
    ...
    override fun add(comp: Component, weightX: Number?, weightY: Number?): Component {
        val gbc = gbc.clone() as GridBagConstraints
        if(weightX != null) gbc.weightx = weightX.toDouble()
        if(weightY != null) gbc.weighty = weightY.toDouble()
        add(comp, gbc)
