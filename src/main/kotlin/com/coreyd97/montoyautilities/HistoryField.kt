...
class HistoryField(
    ...
    private val validateInput: (String) -> (String?) = { s -> s },
    private val onChange: (String) -> Unit = {}
) : JComboBox<String?>() {
    private val history: LinkedHashSet<String> by Preference(preferenceKey, LinkedHashSet())
    ...
    private fun configureComponent() {
        ...
        this.addItemListener { e: ItemEvent ->
            if (e.stateChange == ItemEvent.SELECTED) {
                ...
                if(!suppressEvents){
                    val overruled = validateInput(proposed) ?: return@addItemListener
                    (model as HistoryComboModel).addToHistory(overruled)
                    selected = overruled
                    onChange(overruled)
                }else{
